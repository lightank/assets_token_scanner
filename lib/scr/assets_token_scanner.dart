...
// 修改点：全局替换 r.dart 为 token.dart，默认 className = 'R' 替换为 className = 'Token'
class _AssetsScannerOptions {
  ...
  factory _AssetsScannerOptions.fromYamlMap(YamlMap map) {
    return _AssetsScannerOptions._(
        ...
        className: map['className'] as String? ?? 'Token',
        ignoreComment: map['ignoreComment'] as bool? ?? false,
        namePattern: map['namePattern'] as String?);
  }

  /// The path where the `token.dart` file locate. Note that the `path` should be
  /// sub-path of `lib/`.
  final String path;

  /// The class name of the `token.dart`.
...
/// a `token.dart` with `const` properties of assets path by default. You can custom
...
class AssetsBuilder extends Builder {
  ...
  FutureOr<void> build(BuildStep buildStep) async {
    ...
    final output =
        AssetId(buildStep.inputId.package, p.join(dir, 'token.dart'));
    ...
    if (assetPaths.isNotEmpty) {
      ...
      for (final assetPath in assetPaths) {
        ...
        if (propertyName.isNotEmpty &&
          ...
          assetPathsClass
            // 修改点：assetPath 替换成 propertyName
            ..writeln('  static const $propertyName = \'$propertyName\';')
            // ..writeln('  static const $propertyName = \'$assetPath\';')
            ..writeln();
        }
      }

      // 修改点：注释掉 static String? get(String fileName)
      /*
  ...
  /// Create [_AssetsScannerOptions] from `assets_token_scanner_options.yaml` file
  _AssetsScannerOptions _getOptions() {
    final optionsFile = File('assets_token_scanner_options.yaml');
